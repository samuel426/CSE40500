...
class BiLSTMModel(nn.Module):
    ...
    def forward(self, x, h0_f=None, c0_f=None, h0_b=None, c0_b=None):
        x = x.squeeze(1)  # (B, 1, 32, 5) → (B, 32, 5)


        if h0_f is None:
            h0_f = torch.zeros(1, x.size(0), 32).to(x.device)
        if c0_f is None:
            c0_f = torch.zeros(1, x.size(0), 32).to(x.device)
        if h0_b is None:
            h0_b = torch.zeros(1, x.size(0), 32).to(x.device)
        if c0_b is None:
            c0_b = torch.zeros(1, x.size(0), 32).to(x.device)

            
        # Forward LSTM
        _, (h_f, _) = self.forward_lstm(x, (h0_f, c0_f))

        # Backward LSTM
        _, (h_b, _) = self.backward_lstm(torch.flip(x, [1]), (h0_b, c0_b))
...
def train(model, train_loader, val_loader, save_path):
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        for x, y in train_loader:
            ...
            h0_f = torch.zeros(1, x.size(0), 32).to(DEVICE)
            c0_f = torch.zeros(1, x.size(0), 32).to(DEVICE)
            h0_b = torch.zeros(1, x.size(0), 32).to(DEVICE)
            c0_b = torch.zeros(1, x.size(0), 32).to(DEVICE)
