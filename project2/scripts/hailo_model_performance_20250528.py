...
class NPUPerformanceMeasurer:
    ...
    def calculate_rmse_with_npu(self, hef_path: str, input_data: Dict[str, np.ndarray], 
                               sequences: np.ndarray, data_min: np.ndarray, data_max: np.ndarray) -> Optional[float]:
        """
        NPU를 사용하여 실제 추론을 실행하고 RMSE 계산 (기존 패턴 동일하게)
        
        Args:
            hef_path: HEF 파일 경로
            input_data: 입력 데이터 딕셔너리
            sequences: 원본 시퀀스 데이터
            data_min, data_max: 정규화 파라미터
        ...
        try:
            ...
            with VDevice(params) as vdevice:
                ...
                with infer_model.configure() as configured_model:
                    ...
                    # 테스트할 시퀀스 수 (처음 50개만 - 빠른 테스트용)
                    test_sequences = min(50, len(sequences))
                    logger.info(f"Testing {test_sequences} sequences for RMSE calculation...")
                    
                    for i in range(test_sequences):
                        try:
                            # 현재 시퀀스로 입력 데이터 업데이트
                            current_seq = sequences[i:i+1]  # (1, 8, 5)
                            model_name = list(input_data.keys())[0].split('/')[0]
                            
                            if "GRU" in model_name:
                                # GRU: input_x와 input_h0 업데이트
                                input_x = current_seq.reshape(1, 1, 8, 5)
                                input_h0 = np.zeros((1, 1, 1, 5), dtype=np.uint8)
                                
                                # 기존 버퍼에 새 데이터 복사
                                np.copyto(bindings.input(f"{model_name}/input_layer1").get_buffer(), input_x)
                                np.copyto(bindings.input(f"{model_name}/input_layer2").get_buffer(), input_h0)
                            else:
                                # LSTM: input_x만 업데이트
                                input_x = current_seq.reshape(1, 1, 8, 5)
                                np.copyto(bindings.input(f"{model_name}/input_layer1").get_buffer(), input_x)
                            ...
                            output_data = output_buffers[output_name].copy()
                            
                            # 예측 결과 처리
                            pred_normalized = output_data.astype(np.float32) / 255.0
                            pred_original = pred_normalized * (data_max - data_min) + data_min
                            
                            # Close 가격 추출 (출력 형태에 따라 조정)
                            if pred_original.size >= 5:
                                # 5개 OHLCV 값이 있다면 Close (4번째) 추출
                                pred_close = pred_original.flatten()[3] if pred_original.ndim > 1 else pred_original[3]
                            elif pred_original.size == 1:
                                # 단일 값이라면 그것이 Close 예측값
                                pred_close = pred_original.flatten()[0]
                            else:
                                logger.warning(f"Unexpected output shape: {pred_original.shape}")
                                continue
                            
                            predictions.append(pred_close)
                            
                            # 실제 다음 Close 가격 (간단하게 다음 시퀀스의 마지막 Close 사용)
                            if i + 1 < len(sequences):
                                next_seq = sequences[i + 1]  # (8, 5)
                                actual_normalized = next_seq[-1, 3]  # 마지막 timestep의 Close
                                actual_original = (actual_normalized / 255.0) * (data_max[0, 3] - data_min[0, 3]) + data_min[0, 3]
                                actual_values.append(actual_original)
                            else:
                                # 실제값이 없으면 예측값도 제거
                                predictions.pop()
                            ...
                            # 진행상황 출력
                            if (i + 1) % 10 == 0:
                                logger.info(f"  RMSE progress: {i + 1}/{test_sequences}")
                                
                        except Exception as e:
                            logger.warning(f"Error in sequence {i}: {e}")
                            # 에러 발생시 해당 예측값 제거
                    ...
                    # RMSE 계산
                    if len(predictions) > 0 and len(actual_values) > 0 and len(predictions) == len(actual_values):
                        ...
                        logger.info(f"RMSE calculated from {len(predictions)} predictions")
                        logger.info(f"Sample predictions: {predictions[:3]}")
                        logger.info(f"Sample actual values: {actual_values[:3]}")
                        logger.info(f"Calculated RMSE: {rmse:.4f}")
                        
                        return float(rmse)
                    else:
                        logger.warning(f"Invalid data for RMSE: predictions={len(predictions)}, actual={len(actual_values)}")
                        return None
        
        except Exception as e:
            logger.error(f"Error calculating RMSE with NPU: {e}")
            import traceback
            traceback.print_exc()
            return None
    ...
    def measure_single_model(self, stock: str, model_type: str) -> Dict:
        ...
        # 3. 메모리 모니터링 (추론과 동시에) - 기존 방식 그대로 유지
        ...
        # 추론을 약간 실행하여 메모리 사용량 측정 (기존 코드 그대로)
        try:
        ...
        # 4. RMSE 계산 (완전히 별도로 실행)
        logger.info("=== Calculating RMSE ===")
        rmse = self.calculate_rmse_with_npu(str(hef_path), input_data, sequences, data_min, data_max)
    ...
    def print_summary(self):
        ...
        for result in self.results.values():
            ...
            if result and result.get('rmse') is not None:
                all_rmses.append(result['rmse'])
        ...
        if all_rmses:
            ...
            print(f"  RMSE Range: {np.min(all_rmses):.4f} - {np.max(all_rmses):.4f}")
